...
phases = np.array([f.phase for f in swarm.agents])
norm = plt.Normalize(vmin=0, vmax=swarm.threshold)

sc = ax.scatter(
    positions[:, 0], positions[:, 1], s=100,
    c=phases, cmap='YlOrBr', norm=norm,
...
# Add colorbar as a legend for phase
...
def animate(frame):
    ...
    flash_count_history.append(sum(swarm.get_flash_states()))
